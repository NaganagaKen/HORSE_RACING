...
## これからやること
- ベースライン作成
  - 1着かそれ以外かを予測する2クラス分類にする（不均衡なため工夫が必要）
  - データリークに気を付ける
    - レース前に知ることが出来ない情報が特徴量として含まれている。
    - 時系列データのためfoldの分割方法に注意する。
  - キャリブレーションを用いて予想確率を補正
  - 期待値計算、馬券購入の戦略は後回し。
    - ポートフォリオを組むことになるか。
- 特徴量エンジニアリング
  - 中何週か（日時と馬の名前から計算できる）
  - mother, fatherなどの重要そうなカテゴリデータはtf-idfでベクトル化する（bertでも可）
...
- 外部データの活用（全員同じ条件だから予測精度に影響がなさそう）
  - 気温
  - 湿度
  - 風向きと強さ
