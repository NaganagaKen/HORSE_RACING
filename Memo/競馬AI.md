...
- [済] 過去に選択した脚質の累積和を計算（出走回数で割り正規化する）
  - 出走回数も追加しておく
- [済] 距離を短距離、マイル、中距離、長距離に分類（ウマ娘に合わせるか、実際の競馬に合わせるか決める）
  - これで特徴量を作ってみる
...
- [済] 各馬の使っていないリーク情報の平均を計算（クラスコードなどでグループ化して計算してみる）
  - 平均着差
  - 平均タイム差
- [済] 坂の有無を特徴量として加える（どの競馬場にも坂はある！！！）
- [ ] 一通り特徴量を追加したらSHAPを使ってみる

### 7/2 やること
- [ ] features.py の整理
  - デバッグしやすいように処理を関数でまとめて、main関数で一気に処理するようにする
  - 未定義の動作をした場合にはちゃんとエラーを吐くようにする
    - feature_colに同じcolが2つ以上ある
...
- [ ] 多クラス分類(1, 2, 3着とそれ以外)用のlightGBMモデルを作成
...
6/26
  ...
  - 修正案
    ...
    - [ ] optunaの目的関数の変更
      - oof全体のloglossを最小化していたが、単勝の期待値が1を超えるような馬のみに絞ってからloglossを計算する