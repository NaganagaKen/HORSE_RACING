...
### 今日やること
- Elo Rating と Glicko2を埋め込み（馬と騎手の両方を計算）（済）
- 各レーティングの上昇度を特徴量として追加し、EloRatingの上昇度を用いてグルーピング(calc_group_rating)を行う
- 過去に選択した脚質の累積和を計算（出走回数で割り正規化する）
  - 出走回数も追加しておく
- 距離を短距離、マイル、中距離、長距離に分類（ウマ娘に合わせるか、実際の競馬に合わせるか決める）
- prizeの累積和をfatherで計算（リーディングザイアーを計算）
  - うまく行ったらmotherやboodmaresireでも計算してみる
- optunaの目的関数を訂正（不均衡（勝ち馬 1 : 負け馬 N-1）も相まって「勝ち馬を外すペナルティが相対的に小さい」という問題が起きやすいので重みづけを行う）
- レーティング同士の相互作用特徴量を計算
- 各馬の使っていないリーク情報の平均を計算（クラスコードなどでグループ化して計算してみる）
  - 平均着差
  - 平均タイム差
- 坂の有無を特徴量として加える
- 一通り特徴量を追加したらSHAPを使ってみる
...
6/26
  - 単勝の期待値が1を超える馬のみでキャリブレーションカーブを描くと、均等線の下をカーブが移動してしまう
  - 勝率を過大評価しているから期待値が1を超えてしまう？
  - 勝率の高い馬を過大評価しているのではなく、勝率の低い馬を過小評価している？
  - 修正案
    - キャリブレーション -> キャリブレーションカーブの改善がみられる（画像）。（済）
    - 特徴量エンジニアリングが更に必要そう（レース展開の予測、血統情報の追加等）
    - 学習時にhorse_Nの逆数で重み付けを行う（済）
    - アンサンブル
      - NN（キャリブレーション必須, Temperature Scaling）
      - ロジスティック回帰